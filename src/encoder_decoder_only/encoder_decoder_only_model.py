...
class EncoderDecoderOnlyModel(nn.Module):
    """
    Simplified decoder-encoder-only version of UnitedNet for unsupervised group identification.
    This model only includes encoders, fusers, projectors, and clusters - removing
    decoders, and translation components.
    """
    def __init__(self, config):
        ...
        # Core components for encoding, decoding and clustering
...
class EncoderDecoderOnlyUnitedNet:
    """
    Simplified interface for encoder-decoder-only UnitedNet model
    """
    ...
    def _create_model_from_config(self, config):
        self.model = EncoderDecoderOnlyModel(config)
    ...
    def train(self, adatas_train, save_path=None, adatas_val=None, init_classify=False, verbose=False):
        ...
        from src.encoder_decoder_only.encoder_decoder_only_scheduler import encoder_decoder_only_run_train
        ...
        encoder_decoder_only_run_train(
    ...
    def evaluate(self, adatas, give_losses=False, stage='train'):
        """Evaluate the model"""
        from src.data import create_dataloader
        from src.encoder_decoder_only.encoder_decoder_only_scheduler import encoder_decoder_only_run_evaluate
        
        dataloader = create_dataloader(self.model, adatas, shuffle=False)
        return encoder_decoder_only_run_evaluate(self.model, dataloader, give_losses=give_losses, stage=stage)
    ...
    def infer(self, adatas):
        ...
        self.model.eval()
        with torch.no_grad():
            from src.encoder_decoder_only.encoder_decoder_only_scheduler import encoder_decoder_only_run_through_dataloader
            outputs = encoder_decoder_only_run_through_dataloader(self.model, dataloader, infer_model=True)
